...
export class AppComponent implements OnInit {
  ...
  constructor(
    ...
    this.actionStore.pipe(select(Reducers.getUser)).subscribe(state => {
      this.personnelId = state.PersonnelId
  ...
  ngOnInit() {
    ...
    this.router.events
      ...
      .subscribe(event => this.titleService.setTitle('SiteBook | ' + event['title']))
    ...
    const checkUser = () => {
      if (this.personnelId && this.companyId) {
        if (!this.store.UserInfo.PersonnelId || !this.store.UserInfo.CompanyId) {
          this.router.navigate(['/auth/login'])
        } else if (
          this.personnelId != this.store.UserInfo.PersonnelId ||
          this.companyId != this.store.UserInfo.CompanyId
        ) {
          this.topbar.refreshConnections()
